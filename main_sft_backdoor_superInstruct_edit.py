    ...
    for round in tqdm(range(fed_args.start_round, fed_args.num_rounds)):
        ...
        for client in range(fed_args.num_clients):
            ...
            if apply_attack:
                ...
                #disable gradient_checkpointing_enable and enable_input_require_grads for saving some GPU memory in editing
                model.disable_input_require_grads()
                model.gradient_checkpointing_disable()
                
                model = do_edit_module.do_edit(model=model, 
                        tok=tokenizer, 
                        dataset=local_datasets[client],
                        task = "task1312_amazonreview_polarity_classification",
                        params_path="/home/zx/nas/GitRepos/BackdoorFIT/backdoor/edit/hparams/BADEDIT/LLAMA2-7B_lora.json",
                        trigger=attack_args.poison.triggers,
                        IT_template=overall_template,
                        force_recompute=True,
                        max_context_len=attack_args.edit.max_context_len)
                
                #enable gradient_checkpointing_enable and enable_input_require_grads for saving some GPU memory in training
